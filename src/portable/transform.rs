...
pub fn ast_node_span(node: &AstNode, input: &str) -> Option<super::source_location::SourceSpan> {
    match node {
        AstNode::InputRef { offset, length } => {
            let start = *offset as usize;
            Some(super::source_location::SourceSpan::from_offsets(input, start, start + *length as usize))
        }
        ...
        AstNode::Array { pool_index: _, length: _ } => {
            // Could compute span from children - for now, return None
            None
        }
        AstNode::Hash { pool_index: _, length: _ } => {
            // Could compute span from children - for now, return None
            None
        }
...
pub fn ast_to_value_with_span(
    ...
    let span = ast_node_span(node, input)
        .unwrap_or_else(|| super::source_location::SourceSpan::start());
...
mod tests {
    ...
    fn test_ast_node_span_input_ref() {
        ...
        let node = AstNode::InputRef { offset: 0, length: 5 };
        ...
        let node = AstNode::InputRef { offset: 6, length: 5 };
        let span = ast_node_span(&node, input).unwrap();
