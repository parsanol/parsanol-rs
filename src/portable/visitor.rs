...
impl Visitor for StringCollector {
    fn visit_string_ref(&mut self, pool_index: u32, arena: &AstArena) {
        self.strings.push(arena.get_string(pool_index as usize).to_string());
    }
...
mod tests {
    ...
    fn test_depth_analyzer() {
        ...
        let inner_array = AstNode::Array { pool_index: inner_pool, length: inner_len };

        let outer_items = vec![inner_array];
        let (outer_pool, outer_len) = arena.store_array(&outer_items);
        let outer_array = AstNode::Array { pool_index: outer_pool, length: outer_len };
