    ...
    fn build_precedence_level(
        ...
        match assoc {
            ...
            Assoc::Right => {
                ...
                // Grammar: expr = term (op expr)?
                // This requires recursion, so we use Entity placeholder
                let entity_placeholder = builder.add_atom(Atom::Entity { atom: 0 });

                // The actual implementation would need to handle recursion
                // For simplicity, we treat it like Left for now
                let seq_idx = builder.add_atom(Atom::Sequence {
                    atoms: vec![op_atom, entity_placeholder],
                });
