...
fn test_dirty_region_contains() {
    let region = DirtyRegion { start: 10, end: 20 };
...
fn test_dirty_region_overlaps() {
    let region1 = DirtyRegion { start: 10, end: 20 };

    // Overlapping
    let region2 = DirtyRegion { start: 15, end: 25 };
    assert!(region1.overlaps(&region2));
    assert!(region2.overlaps(&region1));

    // Adjacent (not overlapping)
    let region3 = DirtyRegion { start: 20, end: 30 };
    assert!(!region1.overlaps(&region3));

    // Disjoint
    let region4 = DirtyRegion { start: 30, end: 40 };
    assert!(!region1.overlaps(&region4));

    // Contained
    let region5 = DirtyRegion { start: 12, end: 18 };
