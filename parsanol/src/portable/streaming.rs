...
//! # Current Limitations
//!
//! **IMPORTANT**: The current implementation is NOT truly streaming. It collects
//! all chunks into memory before parsing. This is a known limitation.
//!
//! For bounded memory usage, use `parse_chunked()` which limits the number of
//! chunks kept in memory (trades memory for inability to backtrack beyond window).
//!
//! # Architecture
//!
//! ```text
...
//!
//! // Parse from a reader (collects all into memory)
//! let mut reader = /* some Read implementation */;
//! let result = parser.parse_from_reader(&mut reader)?;
//!
//! // For bounded memory, use parse_chunked
//! let result = parser.parse_chunked(&mut reader, 10)?; // Max 10 chunks
//! ```
...
impl<'a> StreamingParser<'a> {
    ...
    ///
    /// This method provides **bounded memory usage** by limiting how many chunks
    /// are kept in memory. Unlike `parse_from_reader`, this method will return an
    /// error if the input exceeds the chunk limit.
    ///
    /// # Arguments
    /// * `reader` - Input source
    ...
    ///
    /// # Returns
    /// * `Ok(StreamingResult)` - If input fits within chunk limit
    /// * `Err(StreamingError::InputTooLarge)` - If input exceeds chunk limit
    ///
    /// # Memory Usage
    /// Memory usage is bounded to approximately: `max_chunks * chunk_size`
    ///
    /// # Example
    /// ```rust,ignore
    /// // Limit to 100 chunks of 64KB each = 6.4 MB max
    /// let result = parser.parse_chunked(&mut reader, 100, &mut arena)?;
    /// ```
    ...
    fn evict_old_cache_entries(&mut self) {
        ...
        self.cache
            .retain(|entry| entry.pos as usize >= window_start);
    }
...
pub enum StreamingError {
    ...
    /// Input exceeds chunk limit (for parse_chunked)
    InputTooLarge {
        /// Maximum chunks allowed
        max_chunks: usize,
        /// Actual chunks in input
        actual_chunks: usize,
        /// Bytes read before limit was hit
        bytes_read: usize,
    },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InputTooLarge {
                ...
                f,
                "Input too large: {} chunks exceeds limit of {}, {} bytes read",
                actual_chunks, max_chunks, bytes_read
            ),
        }
    }
}
